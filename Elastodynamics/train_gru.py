...
def count_parameters(model):
    ...
    total_params = jax.tree_util.tree_reduce(
        lambda acc, x: acc + count_array(x),
        params,
        initializer=0
    )
...
@hydra.main(config_path="configs", config_name="config_gru", version_base=None)
def main(cfg: DictConfig):
    ...
    parent_results_folder = cfg.paths.results
    ...
    def create_training_round_folder(parent_folder):
        ...
        round_folder = os.path.join(parent_folder, f"training_round_{slurm_id}_gru_fullfield_mse{lookahead_suffix}")
    ...
    class GRUBranch(eqx.Module):
        ...
        def __init__(self, input_size, hidden_size, output_size, num_layers=1, *, key, use_layer_norm=True):
            ...
            if use_layer_norm:
                self.norms = [eqx.nn.LayerNorm(hidden_size) for _ in range(num_layers)]
            else:
                self.norms = None
        ...
        def __call__(self, xs):
            ...
            def scan_fn(states, x_t):
                ...
                for i in range(self.num_layers):
                    inp = x_t if i == 0 else new_states[i-1]
                    
                    new_state = self.gru_cells[i](inp, states[i])
                    
                    if self.norms is not None:
                        new_state = self.norms[i](new_state)
                    
                    new_states.append(new_state)
                
                return new_states, new_states[-1]
            
            init_states = [jnp.zeros(self.hidden_size) for _ in range(self.num_layers)]
            

            final_states, outputs = jax.lax.scan(scan_fn, init_states, xs)
            

            temporal_features = jax.vmap(self.readout)(outputs)
            
            aggregated_features = jnp.mean(temporal_features, axis=0)
            
            output = aggregated_features.reshape(branch_hidden_size, num_output_components)
            
            return output

    class GRU_DeepONet(eqx.Module):
        branch_net: GRUBranch
        trunk_net: MLP

        def __init__(self, *, key):
            keys = jr.split(key, 2)
            
            self.branch_net = GRUBranch(
                input_size=branch_feature_in,
                hidden_size=branch_hidden_size,
                output_size=branch_hidden_size * num_output_components,
    ...
    model = GRU_DeepONet(key=model_key)
    ...
    def save_model(model, epoch, subdirectory):
        ...
        model_filename = pathlib.Path(subdirectory) / f'gru_deeponet_trained_model_epoch_{epoch}.eqx'
    ...
    for idx, label in zip(selected_indices, labels):
        ...
        for t in time_steps:
            ...
            for c in range(num_components):
                ...
                plt.legend()
            fname = f"sample_{idx}_{label}_time_{t}_gru.png"
            plt.tight_layout()
    ...
    for t in sel_ts:
        for c in range(num_components):
            ...
            plt.tight_layout()
            fname = f"heatmap_{comp_names[c]}_sample{median_sample_idx}_t{t}_gru.png"
            plt.savefig(os.path.join(training_round_folder, fname))
            plt.close()
    ...
    metrics_filename = "evaluation_metrics_gru_fullfield_mse_lookahead.yaml" if use_lookahead else "evaluation_metrics_gru_fullfield_mse.yaml"
