...
def main(cfg: DictConfig):
    ...
    round_dir = pathlib.Path(parent_res) / f"training_round_{slurm_id}_{ts}_fullfield_mse{lookahead_suffix}"
    ...
    print("Pre-computing trunk input grid...")
    ...
    def compute_full_field_loss(model, x_branch_batch, y_true_batch, trunk_inputs_flat):
        ...
        predictions = predictions.reshape(batch_size, n_coords, time_steps, 1)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (xb, yb) in enumerate(dataloader()):
            ...
            loss_mse, model, opt_state = step(model, xb, yb, trunk_inputs_flat, opt_state)
    ...
    def evaluate_model_full_field(model, x_test, y_test, trunk_inputs, n_samples=10):
        ...
        for i in range(n_samples):
            ...
            preds[i] = np.asarray(pred.reshape(n_coords, n_t, 1))
    ...
    print(f"  Full field evaluation: {n_coords} coords × {HD_interact} time steps")
