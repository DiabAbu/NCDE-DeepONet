#!/usr/bin/env python
# ====================================================================
#  Thermoelasticity NCDE-DeepONet
# ====================================================================
...
def main(cfg: DictConfig):
    ...
    with h5py.File(os.path.join(data_dir, 'thermoelasticity_dataset_fixed.h5'), 'r') as f:
        x_train = jnp.array(f['data/X_train'][:], dtype=float_dtype)
        y_train = jnp.array(f['data/Y_train'][:], dtype=float_dtype)
        x_test  = jnp.array(f['data/X_test'][:],  dtype=float_dtype)
        y_test  = jnp.array(f['data/Y_test'][:],  dtype=float_dtype)
        coords  = jnp.array(f['data/coordinates'][:, :2], dtype=float_dtype)
    ...
    y_train = jnp.moveaxis(y_train, 1, 2)
    y_test  = jnp.moveaxis(y_test, 1, 2)
    ...
    def scale_data_inplace(x_train, x_test, y_train, y_test):
        ...
        if normalization_method == "standard":
            ...
            scale_factors = {
                'x': [np.max(np.abs(x_train_np[:, :, i])) for i in range(3)],
                'y': [np.max(np.abs(y_train_np[:, :, :, i])) for i in range(3)] 
            }
    ...
    memory_per_sample = field_size * 3 * 4  
    ...
    lw2 = cfg.training.get("loss_weight_component_2", 1.0)  
    loss_weights = jnp.array([lw0, lw1, lw2], dtype=float_dtype)
    print(f"Using loss weights: ux = {lw0}, uy = {lw1}, T = {lw2}")
    ...
    def compute_full_field_loss(model, x_branch_batch, y_true_batch, trunk_inputs_flat):
        ...
        predictions = predictions.reshape(batch_size, n_coords, time_steps, 3)
    ...
    if test_metrics_history:
        ...
        val_mae = np.array([m["mae_total"] for m in test_metrics_history], np.float32)
    ...
    zoom_start = int(0.75 * len(loss_arr))
    ax3.plot(steps_all[zoom_start:], loss_arr[zoom_start:])
    ...
    evaluation_info = {
        ...
        "loss_function": "MSE",
        "loss_weights": {"ux": lw0, "uy": lw1, "T": lw2},
    ...
    print(f"  Loss weights: ux={lw0}, uy={lw1}, T={lw2}")
